...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        account_mapping_checkpoints_prefix = os.getenv('ACCOUNT_MAPPING_CHECKPOINTS_DIR')
        base_data_dir = os.getenv('ACCOUNT_MAPPING_BASE_DATA_DIR')
        account_mapping_api_checkpoints_dir = os.getenv('ACCOUNT_MAPPING_API_CHECKPOINTS_DIR')
        model_data = model_collection_step(base_data_dir=base_data_dir,
                                           bucket_name=bucket_name,
                                           checkpoints_prefix=account_mapping_checkpoints_prefix,
                                           checkpoints_dir=account_mapping_api_checkpoints_dir,
                                           logger=logger)
...
def get_llm_context_data() -> Dict[str, Dict[str, Any]]:
    """
    Retrieve LLM context data for account mapping inference.

    Loads and returns the LLM context data required for account mapping predictions.
    ...
    Dict[str, Dict[str, Any]]
        ...
        tenant-specific context for account mapping inference.
...
@app.post("/account_mapping", response_model=JournalLinePredictionResponse)
def account_mapping(request: JournalLinePredictionRequest) -> JournalLinePredictionResponse:
    """
    Perform account mapping for a batch of journal records using the KNN model.
...
@app.post("/llm_account_mapping", response_model=LLMJournalLinePredictionResponse)
async def llm_account_mapping(request: JournalLinePredictionRequest) -> LLMJournalLinePredictionResponse:
    """
    HTTP endpoint for LLM-based account mapping prediction using journal line data.

    This function validates tenant and table context, converts request data to DataFrame format,
    initializes the LLM client, and performs inference to predict account mappings for journal lines.

    Parameters
    ----------
    request : JournalLinePredictionRequest
        The request object containing tenant_id, table name, and journal line data records
        to be processed for account mapping prediction.
